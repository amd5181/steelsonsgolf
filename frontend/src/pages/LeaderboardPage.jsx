...
const PaymentDot = ({ paid }) => paid
  ? <span className="flex-shrink-0 w-2 h-2 rounded-full bg-emerald-500" title="Paid" />
  : <span className="flex-shrink-0 w-2 h-2 rounded-full bg-red-500 animate-pulse" title="Unpaid" />;
        ...
        <div className={`divide-y divide-white/5 ${expandedStandings ? 'max-h-[500px] overflow-y-auto' : ''}`}>
          ...
          {displayScores.map((g, i) => (
            <div key={i} className="flex items-center px-4 py-2 gap-2">
              ...
              <span className={`font-numbers font-bold text-sm w-8 text-right flex-shrink-0 ${g.total_score?.toString().startsWith('-') ? 'text-[#CCFF00]' : 'text-white/70'}`}>{g.total_score}</span>
              <span className="font-numbers text-xs text-white/40 w-8 text-right flex-shrink-0">{g.fantasy_points > 0 ? g.fantasy_points : ''}</span>
            </div>
          ))}
        </div>
      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          className="w-full pl-8 pr-7 py-1.5 rounded-lg border border-slate-200 bg-white text-base sm:text-sm text-[#0F172A] placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#1B4332]/30 focus:border-[#1B4332]/40 transition-all"
                        />
                        {search && (
                          <button
                            onClick={() => setSearch('')}
